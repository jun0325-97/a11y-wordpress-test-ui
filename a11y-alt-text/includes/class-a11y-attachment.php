...
class A11Y_Attachment {
  ...
  private function normalize_lang( $lang, $attachment_id, $supported_languages ) {
    ...
    if ( ! is_string( $lang ) || '' === trim( $lang ) ) {
      return A11Y_Utility::lang_for_attachment( $attachment_id );
    }
   ...
   *
   *                                 Note: Global 'a11y_force_lang' setting will override 'lang' if enabled.
   *
  ...
  public function generate_alt( $attachment_id, $attachment_url = null, $options = [] ) {
    ...
    $api_options = wp_parse_args(
      ...
      array(
        ...
        'lang'              => A11Y_Utility::lang_for_attachment( $attachment_id ),
      )
    );
    ...
    if ( $attachment_id ) {
      ...
      $attachment_url = apply_filters( 'a11y_attachment_url', $attachment_url, $attachment_id );
      ...
      if ( ! count( $api_options['keywords'] ) ) {
        ...
        if ( ! count( $api_options['keywords'] ) && ( A11Y_Utility::get_setting( 'a11y_keywords_title' ) === 'yes' ) ) {
    ...
    /**
     * Filter API options before sending to the A11Y API.
     *
     ...
     */
    $api_options = apply_filters( 'a11y_before_create_image_options', $api_options, $attachment_id, $attachment_url );
    ...
    $supported_languages = A11Y_Utility::supported_languages();

    // Normalize language (ensure a default even if a filter removed it)
    $api_options['lang'] = $this->normalize_lang(
      $api_options['lang'] ?? A11Y_Utility::lang_for_attachment( $attachment_id ),
      $attachment_id,
      $supported_languages
    );

    // Enforce force_lang setting if enabled (overrides filter and caller language)
    if ( 'yes' === A11Y_Utility::get_setting( 'a11y_force_lang' ) ) {
      $forced_lang = A11Y_Utility::get_setting( 'a11y_lang', A11Y_Utility::get_default_language() );
    ...
    $response_code = null;
    $max_retries = apply_filters( 'a11y_max_retries', 3 ); // Default 3 retries for admin-AJAX responsiveness
    $delay = 1; // 1 second
    $start_time = microtime( true );
    $time_budget = apply_filters( 'a11y_retry_time_budget', 12 ); // Maximum seconds for all retries
    ...
    for ($attempt = 0; $attempt < $max_retries; $attempt++) {
      ...
      $hard_fail_codes = apply_filters( 'a11y_hard_fail_http_codes', array( 400, 401, 403, 404, 422 ) );
      ...
      $retryable_codes = apply_filters( 'a11y_retryable_http_codes', array( 429, 503, 504, 408 ) );
  ...
  public function is_attachment_eligible( $attachment_id, $context = 'generate', $dry_run = false ) {
    ...
    if ( defined( 'A11Y_TESTING' ) && A11Y_TESTING ) {
      return true;
    }
    ...
    $custom_skip = apply_filters( 'a11y_skip_attachment', false, $attachment_id );
    if ( $custom_skip ) {
      return false;
    }
    ...
    if ( $this->is_attachment_excluded_by_post_type( $attachment_id ) ) {
      if ( $should_log ) {
        ...
        A11Y_Utility::log_error(
          sprintf(
            '<a href="%s" target="_blank">Image #%d</a>: %s (%s)',
            esc_url($attachment_edit_url),
            (int) $attachment_id,
            esc_html__('Excluded post type in user settings.', 'a11y-alt-text'),
    ...
    if ( empty( $meta ) && file_exists( $file ) ) {
      if ( $dry_run || ( A11Y_Utility::get_setting( 'a11y_wp_generate_metadata' ) === 'no' ) ) {
    ...
    if (!$size) {
       ...
       * Example usage:
       * ```php
       * add_filter( 'a11y_attachment_size', function( $size, $attachment_id ) {
       ...
       */
      $custom_attachment_size = apply_filters( 'a11y_attachment_size', null, $attachment_id );
      if ( $custom_attachment_size !== null ) {
        ...
        if ( isset( $offload_meta['key'] ) ) {
          $size = A11Y_Utility::get_attachment_size( $attachment_id );
        }
      }
    }
    ...
    $file_type_extensions = A11Y_Utility::get_setting( 'a11y_type_extensions' );
    $attachment_edit_url = get_edit_post_link($attachment_id);
        ...
        if ( $should_log ) {
          A11Y_Utility::log_error(
            sprintf(
              ...
              esc_url($attachment_edit_url),
              (int) $attachment_id,
              esc_html__('User setting image filtering: Filetype not allowed.', 'a11y-alt-text'),
              esc_html($extension)
            )
          );
        }
      ...
      if ( $should_log ) {
        A11Y_Utility::log_error(
          sprintf(
            ...
            esc_url($attachment_edit_url),
            (int) $attachment_id,
            esc_html__('Unsupported extension.', 'a11y-alt-text'),
            esc_html($extension)
          )
        );
      }
    ...
    if (!$is_svg && $size_unavailable) {
      if ($should_log && A11Y_Utility::get_setting('a11y_skip_filenotfound') === 'yes') {
        A11Y_Utility::log_error(
          sprintf(
            '<a href="%s" target="_blank">Image #%d</a>: %s %s',
            esc_url($attachment_edit_url),
            (int) $attachment_id,
            esc_html__('File not found.', 'a11y-alt-text'),
            esc_html__('(System Issue)', 'a11y-alt-text')
          )
        );
      }
      return false;
      ...
      if ( $should_log ) {
        A11Y_Utility::log_error(
          sprintf(
            ...
            esc_url($attachment_edit_url),
            (int) $attachment_id,
            esc_html__('File size exceeds 16MB limit.', 'a11y-alt-text'),
            $size
          )
        );
      }
      ...
      if ( $should_log ) {
        A11Y_Utility::log_error(
          sprintf(
            ...
            esc_url($attachment_edit_url),
            (int) $attachment_id,
            esc_html__('Image dimensions too small.', 'a11y-alt-text'),
            $width,
            $height
          )
        );
  ...
  public function get_ecomm_data( $attachment_id, $product_id = null ) {
    if ( ( A11Y_Utility::get_setting( 'a11y_ecomm', 'yes' ) === 'no' ) || ! A11Y_Utility::has_woocommerce() ) {
      return array();
    }

    if ( A11Y_Utility::get_setting( 'a11y_ecomm_title' ) === 'yes' ) {
...
SQL;
  ...
  public function filtered_ecomm_data($attachment_id, $ecomm_data) {
     ...
     */
    $ecomm_data = apply_filters( 'a11y_ecomm_data', $ecomm_data, $attachment_id );
    return $ecomm_data;
  }
    ...
    public function get_seo_keywords( $attachment_id, $explicit_post_id = null ) {
      if ( ( A11Y_Utility::get_setting( 'a11y_keywords', 'yes' ) === 'no' ) ) {
        return array();
      }
       ...
       *
       * EXAMPLE USAGE:
         function custom_a11y_seo_keywords($keywords, $attachment_id, $post_id) {
         ...
         add_filter('a11y_seo_keywords', 'custom_a11y_seo_keywords', 10, 3);
       *
       */
      $keywords = apply_filters( 'a11y_seo_keywords', $keywords, $attachment_id, $post_id );
...
SQL;
    ...
    public function theseoframework_seo_keywords( $attachment_id, $post_id ) {
      ...
      if ( ! A11Y_Utility::has_theseoframework() ) {
        return array();
      }
...
SQL;
  ...
  public function smartcrawl_seo_keywords($attachment_id, $post_id)
  {
    ...
    if (! A11Y_Utility::has_smartcrawl()) {
      return array();
    }
  ...
  public function add_attachment( $attachment_id ) {
    if ( A11Y_Utility::get_setting( 'a11y_enabled', 'yes' ) === 'no' ) {
      return;
    }
  ...
  private function get_wpml_enabled_languages_selection() {
    $enabled_languages = A11Y_Utility::get_setting( 'a11y_wpml_enabled_languages', null );
  ...
  public function ajax_bulk_generate() {
    check_ajax_referer( 'a11y_bulk_generate', 'security' );
    ...
    $skip_reasons = get_transient('a11y_bulk_skip_reasons_' . get_current_user_id()) ?: array();
    $redirect_url = admin_url( 'admin.php?page=a11y-bulk-generate' );
    $recursive = true;
...
SQL;
    ...
    if ( $post_id ) {
      ...
      if ( $only_new === '1' ) {
        ...
        $images_to_update_sql = $images_to_update_sql . " AND (NOT EXISTS(SELECT 1 FROM {$a11y_asset_table} WHERE wp_post_id = p.ID))";
      }
    ...
    if ( count( $images_to_update ) == 0 ) {
      ...
      $final_skip_reasons = get_transient('a11y_bulk_skip_reasons_' . get_current_user_id()) ?: array();
      
      // Clean up the transient
      delete_transient('a11y_bulk_skip_reasons_' . get_current_user_id());
      
      
      // Determine appropriate completion message based on context
      $completion_message = $last_post_id > 0 
        ? __( 'Bulk generation complete! All remaining images have been processed.', 'a11y-alt-text' )
        : __( 'All images already have alt text or are not eligible for processing.', 'a11y-alt-text' );
      ...
      if ( $total_skipped > 0 ) {
        ...
        if ( isset( $final_skip_reasons['ineligible'] ) && $final_skip_reasons['ineligible'] > 0 ) {
          $reason_messages[] = sprintf(__('%d ineligible (size/format/settings)', 'a11y-alt-text'), $final_skip_reasons['ineligible']);
        }
        if ( isset( $final_skip_reasons['api_error'] ) && $final_skip_reasons['api_error'] > 0 ) {
          $reason_messages[] = sprintf(__('%d API errors', 'a11y-alt-text'), $final_skip_reasons['api_error']);
        }
        if ( isset( $final_skip_reasons['generation_failed'] ) && $final_skip_reasons['generation_failed'] > 0 ) {
          $reason_messages[] = sprintf(__('%d generation failures', 'a11y-alt-text'), $final_skip_reasons['generation_failed']);
        }
        if ( isset( $final_skip_reasons['wpml_language_filtered'] ) && $final_skip_reasons['wpml_language_filtered'] > 0 ) {
          $reason_messages[] = sprintf(__('%d skipped by WPML language filter', 'a11y-alt-text'), $final_skip_reasons['wpml_language_filtered']);
        }
        
        if ( ! empty( $reason_messages ) ) {
          $subtitle = sprintf(__('Skip reasons: %s', 'a11y-alt-text'), implode(', ', $reason_messages));
        }
      }
    ...
    foreach ( $images_to_update as $image ) {
      ...
      if ( defined( 'A11Y_BULK_DEBUG' ) ) {
        A11Y_Utility::log_error( sprintf("BulkGenerate: Attachment ID %d", $attachment_id) );
      }
      ...
      if ( ! $this->is_attachment_eligible( $attachment_id, 'bulk' ) ) {
        ...
        if ( defined( 'A11Y_BULK_DEBUG' ) ) {
          A11Y_Utility::log_error( sprintf("BulkGenerate: Skipped attachment ID %d (not eligible)", $attachment_id) );
        }
      ...
      if ( $response === 'insufficient_credits' ) {
        ...
        wp_send_json( array(
          'status'      => 'success',
          'message'     => __( 'Images partially updated (no more credits).', 'a11y-alt-text' ),
      ...
      if ( $response === 'url_access_error' ) {
        ...
        wp_send_json( array(
          'status'      => 'error',
          'message'     => __( 'Unable to access image URLs. Your images may be on a private server.', 'a11y-alt-text' ),
    ...
    set_transient('a11y_bulk_skip_reasons_' . get_current_user_id(), $skip_reasons, 3600);
    ...
    if ( $images_skipped > 0 ) {
      $message = sprintf(__('Batch processed: %d updated, %d skipped', 'a11y-alt-text'), $images_successful, $images_skipped);
    } else {
      $message = sprintf(__('Batch processed: %d updated', 'a11y-alt-text'), $images_successful);
    }
  ...
  public function action_single_generate() {
    ...
    if ( ! isset( $_GET['a11y_action'] ) || $_GET['a11y_action'] !== 'generate' ) {
      return;
    }
    ...
    if ( ! current_user_can( 'upload_files' ) ) {
      wp_die( esc_html__( 'You do not have permission to manage attachments.', 'a11y-alt-text' ) );
    }

    // Check user can edit this specific attachment
    if ( ! current_user_can( 'edit_post', $attachment_id ) ) {
      wp_die( esc_html__( 'You do not have permission to edit this attachment.', 'a11y-alt-text' ) );
    }

    // Verify CSRF nonce
    if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'a11y_url_generate' ) ) {
      wp_die(
  ...
  public function ajax_single_generate() {
    check_ajax_referer( 'a11y_single_generate', 'security' );
    ...
    if ( ! current_user_can( 'edit_post', $attachment_id ) ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'You do not have permission to edit this attachment.', 'a11y-alt-text' )
      ) );
    }
    $keywords = is_array($_REQUEST['keywords'] ?? null) ? array_map('sanitize_text_field', $_REQUEST['keywords']) : [];
    ...
    if ( $response === 'insufficient_credits' ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => 'You have no more credits available. Go to your account on A11Y to get more credits.',
      ) );
    }
    ...
    if ( $response === 'url_access_error' ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'Unable to access image URL. Your site may be on a private server.', 'a11y-alt-text' ),
        'action_required' => 'url_access_fix',
      ) );
    }
  ...
  public function ajax_edit_history() {
    check_ajax_referer( 'a11y_edit_history', 'security' );
    ...
    if ( ! $attachment_id ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'Invalid request.', 'a11y-alt-text' )
      ) );
    }
    ...
    wp_send_json( array(
      'status' => 'success',
      'message' => __( 'Alt text updated.', 'a11y-alt-text' )
    ) );
  }
  ...
  private function check_attachment_permissions() {
    if ( ! current_user_can( 'upload_files' ) ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'You do not have permission to manage attachments.', 'a11y-alt-text' )
      ) );
    }
    return true;
  }
  ...
  public function ajax_check_attachment_eligibility() {
    check_ajax_referer( 'a11y_check_attachment_eligibility', 'security' );
    ...
    if ( ! $attachment_id ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'Invalid post ID.', 'a11y-alt-text' )
      ) );
    }
    ...
    if ( ! $this->is_attachment_eligible( $attachment_id, 'check' ) ) {
      wp_send_json( array(
        'status' => 'error',
        'message' => __( 'Image is not eligible for auto-generating ALT text.', 'a11y-alt-text' )
      ) );
    }

    wp_send_json( array(
      'status' => 'success',
      'message' => __( 'Image is eligible for auto-generating ALT text.', 'a11y-alt-text' )
    ) );
  }
  ...
  public function bulk_select_action_handler( $redirect, $do_action, $items ) {
    ...
    $redirect_url = admin_url( 'admin.php?page=a11y-bulk-generate&a11y_action=bulk-select-generate&a11y_batch_id=' . $batch_id );
    return $redirect_url;
  }
  ...
  public function render_bulk_select_notice() {
    ...
    $message = sprintf(
      "[A11Y] Finished generating alt text for %d %s.",
      $count,
      _n(
        'image',
        'images',
  ...
  public function on_translation_created( $post_id, $tr_id, $lang_slug ) {
    ...
    if ( A11Y_Utility::get_setting( 'a11y_enabled', 'yes' ) === 'no' || ! $this->is_attachment_eligible( $tr_id, 'add' ) ) {
      return;
    }
  ...
  public function process_csv() {
    ...
    if ( ! isset( $_POST['a11y_csv_import_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['a11y_csv_import_nonce'] ) ), 'a11y_csv_import' ) ) {
      return array(
        'status' => 'error',
        'message' => __( 'Security check failed. Please refresh the page and try again.', 'a11y-alt-text' )
      );
    }
    ...
    if ( $asset_id_index === false || $alt_text_index === false ) {
      ...
      return array(
        'status' => 'error',
        'message' => __( 'Invalid CSV file. Please make sure the file has the required columns.', 'a11y-alt-text' )
      );
    }
    ...
    if ( empty( $selected_lang ) ) {
      ...
      delete_option( 'a11y_csv_import_lang' );
    } else {
      ...
      if ( $lang_index !== false ) {
        ...
        update_option( 'a11y_csv_import_lang', $selected_lang );
      }
    }
    ...
    $message = __( '[A11Y.so] No images were matched.', 'a11y-alt-text' );

    if ( $images_updated ) {
      $message = sprintf(
        _n(
          '[A11Y.so] Successfully imported alt text for %d image.',
          '[A11Y.so] Successfully imported alt text for %d images.',
          $images_updated,
          'a11y-alt-text'
        ),
        $images_updated
  ...
  public function ajax_preview_csv() {
    check_ajax_referer( 'a11y_preview_csv', 'security' );

    // Check permissions
    if ( ! current_user_can( 'upload_files' ) ) {
      wp_send_json( array(
        'status'  => 'error',
        'message' => __( 'You do not have permission to upload files.', 'a11y-alt-text' ),
      ) );
    }
    ...
    if ( empty( $uploaded_file['tmp_name'] ) ) {
      wp_send_json( array(
        'status'  => 'error',
        'message' => __( 'No file uploaded.', 'a11y-alt-text' ),
      ) );
    }
    ...
    if ( strtolower( $file_type['ext'] ?? '' ) !== 'csv' ) {
      wp_send_json( array(
        'status'  => 'error',
        'message' => __( 'Please upload a CSV file.', 'a11y-alt-text' ),
      ) );
    }
    ...
    $preferred_lang = get_option( 'a11y_csv_import_lang', '' );
  ...
  public function detect_csv_languages( $file_path ) {
    ...
    $supported = A11Y_Utility::supported_languages();
  ...
  public function add_media_alt_filter( $post_type ) {
    ...
    $a11y_filter = sanitize_text_field( $_GET['a11y_filter'] ?? 'all' );

    echo '<select id="filter-by-alt" name="a11y_filter">';
    echo '<option value="all" ' . selected( $a11y_filter, 'all', false ) . '>' . esc_html__( 'Any alt text', 'a11y-alt-text' ) . '</option>';
    echo '<option value="missing" ' . selected( $a11y_filter, 'missing', false ) . '>' . esc_html__( 'Without alt text', 'a11y-alt-text' ) . '</option>';
    echo '</select>';
  }
  ...
  public function media_alt_filter_handler( $query ) {
    ...
    if ( ! is_admin() || ! $query->is_main_query() || ! $is_media_screen || $a11y_filter === 'all' ) {
      return;
    }
  ...
  public function ajax_network_get_stats() {
    check_ajax_referer( 'a11y_network_bulk_generate', 'security' );

    if ( ! current_user_can( 'manage_network_options' ) ) {
      wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'a11y-alt-text' ) ) );
    }
    ...
    do {
      ...
      foreach ( $sites as $site ) {
        ...
        try {
          ...
          error_log( 'A11Y network stats: site ' . $site->blog_id . ' – ' . $e->getMessage() );
          $stats[] = array(
            'blog_id'      => (int) $site->blog_id,
            'name'         => get_bloginfo( 'name' ),
            'url'          => get_bloginfo( 'url' ),
  ...
  public function ajax_network_bulk_generate() {
    ...
    if ( ! $blog_id || ! get_blog_details( $blog_id ) ) {
      wp_send_json_error( array( 'message' => __( 'Invalid site.', 'a11y-alt-text' ) ) );
    }
    ...
    try {
      ...
      if ( null === $images || $wpdb->last_error ) {
        restore_current_blog();
        wp_send_json_error( array( 'message' => __( 'Database query failed.', 'a11y-alt-text' ) ) );
      }
      ...
      if ( empty( $images ) ) {
        ...
        wp_send_json_success( array(
          'message'       => __( 'Site complete.', 'a11y-alt-text' ),
          'process_count' => 0,
          'success_count' => 0,
          'last_post_id'  => $last_post_id,
          'recursive'     => false,
      ...
      foreach ( $images as $image ) {
        ...
        if ( $response === 'insufficient_credits' ) {
          ...
          wp_send_json_success( array(
            ...
            'message'       => __( 'No more credits.', 'a11y-alt-text' ),
            'process_count' => $loop_count,
            'success_count' => $images_successful,
            'last_post_id'  => $last_post_id,
            'recursive'     => false,
      ...
      // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log, QITStandard.PHP.DebugCode.DebugFunctionFound -- Production error logging
      error_log( 'A11Y network bulk generate: blog ' . $blog_id . ' – ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine() );
      restore_current_blog();
      wp_send_json_error( array( 'message' => __( 'An unexpected error occurred.', 'a11y-alt-text' ) ) );
    }
